...
import { Plus, Trash2, RefreshCw, Users, X } from 'lucide-react';
...
function AddListModal({ onClose, onAdd }: { onClose: () => void; onAdd: (l: ContactList) => void }) {
  ...
  const submit = async () => {
    if (!form.name || !form.neon_table_name) { setError('Name and table name required'); return; }
    setLoading(true);
    try {
      const list = await api.contactLists.create(form);
    ...
    <div className="p-8 space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Sourced from your NeonDB tables</p>
        </div>
