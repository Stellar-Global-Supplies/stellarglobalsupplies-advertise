 ...
 * External URLs, javascript:, data:, protocol-relative URLs, etc.
...
function getSafeRedirect(value: string | null): string {
  ...
  try {
    ...
    // Only allow redirects to this exact origin.
    if (target.origin !== window.location.origin) {
      return fallback;
    }

    // Only allow normal HTTP(S) navigation.
    if (
    ...
    // Invalid URL -> safe local fallback.
    return fallback;
  }
}
...
export default function SSOCallback() {
  ...
  useEffect(() => {
    ...
    // SECURITY: Never use the user-controlled redirect directly.
    ...
    if (!token) {
      // redirect is already validated as same-origin.
    ...
    fetch(EXCHANGE_FN, {
      ...
      .then(
        async ({
          ...
          // SECURITY: redirect has already been normalized and
          // verified to belong to window.location.origin.
